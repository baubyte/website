...
return [
    ...
    | reaches the Node SSR container over the internal bridge network by
    | service name, not localhost. `Inertia\Ssr\HttpGateway::dispatch()`
    | already falls back to client-side rendering automatically on any
    | connection failure (refused, timeout, etc.) — see
    | `App\Providers\AppServiceProvider` for the failure-logging listener
    | that observes those fallbacks.
    |
    */
